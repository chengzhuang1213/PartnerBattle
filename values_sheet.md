...
| `assets/ui-panels/` | prep / battle 使用的装饰面板、详情面板、子面板和概念图资源。 |
...
| 出场选择 UI | prep 空状态只显示选择标题和提示文案，不再显示 1/2/3 占位框；实际候补席卡片仍保留局数/顺序标记。 |
...
| 模块 | 当前显示 |
| --- | --- |
| 战斗分析摘要 | 显示关键技能、最高单次伤害、暴击/连击/毒/复活/防反等主要影响、单局胜者；摘要位于日志滚动内容内，会跟随日志一起划走。 |
| 完整日志 | 摘要下方保留逐条事件日志和伤害公式展开；`combat-log-scroll` 负责摘要 + 日志的统一滚动。 |
| 结束复盘 | 战斗结束页显示每局对阵和胜负。 |
